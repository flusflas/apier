...
from apier.templates.python_tree.base.internal.runtime_expr import RuntimeExpressionError, prepare_request
...
def test_prepare_query_params():
    ...
    assert isinstance(result, PreparedRequest)
    assert result.url == 'https://api.test/companies/ibm/department/17/users?foo=bar&foo=abc&limit=20'

    # Set new query param
    req = copy.deepcopy(test_request)
    result = prepare_request(req, '$request.query.value', '123')
    assert isinstance(result, PreparedRequest)
    assert result.url == 'https://api.test/companies/ibm/department/17/users?foo=bar&foo=abc&limit=10&value=123'
...
@pytest.mark.parametrize("req, expression, expected_exception_type", [
